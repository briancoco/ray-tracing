...
class ObjShape{
    ...
    bool hit(Ray* r, glm::vec3& resHP, glm::vec3& resN) {
        ...
        for (size_t i = 0; i < triangles.size(); i++) {
            ...
            if (intersect_triangle(orig, dir, vec0, vec1, vec2, &t, &ba, &bb)) {
                glm::vec3 hitPoint = r->rayOrigin + r->rayDir * (float) t;
                bc = 1 - ba - bb;
                if (first || glm::length(hitPoint - origin) < glm::length(resHP - origin)) {
     
                    float nx = ba * triangles[i]->na.x + bb * triangles[i]->nb.x + bc * triangles[i]->nc.x;
                    float ny = ba * triangles[i]->na.y + bb * triangles[i]->nb.y + bc * triangles[i]->nc.y;
                    float nz = ba * triangles[i]->na.z + bb * triangles[i]->nb.z + bc * triangles[i]->nc.z;

                    resHP = hitPoint;
                    resN = glm::vec3(nx, ny, nz);
